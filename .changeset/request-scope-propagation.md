...
New `Runtime` exports: `readScopeValues`, `readTraceId`, `getScopeHeaders`, `getScopeRequestHeaders`, `exportScope`, `importScope`, `clearScope`, the `ScopeHeaders` and `ScopeSource` types and `MAX_SCOPE_VALUE_LENGTH`. `Runtime.setScope` takes an optional second argument with the header name of each scope key.
